...
test('returns a Date instance', t => {
  t.ok(decode(UTC_TIME, new Buffer('991231235959Z'), 0, 13) instanceof Date)
})

test('decodes UTC_TIME from the 20th century', t => {
  t.same(decode(UTC_TIME, new Buffer('991231235959Z'), 0, 13), new Date('1999-12-31T23:59:59Z'))
})
...
test('decodes GENERALIZED_TIME with 1 fractional second', t => {
  t.same(decode(GENERALIZED_TIME, new Buffer('20160316182035.1Z'), 0, 17), new Date('2016-03-16T18:20:35.1Z'))
})

test('decodes GENERALIZED_TIME with 2 fractional seconds', t => {
  t.same(decode(GENERALIZED_TIME, new Buffer('20160316182035.12Z'), 0, 18), new Date('2016-03-16T18:20:35.12Z'))
})

test('decodes GENERALIZED_TIME with 3 fractional seconds', t => {
  t.same(decode(GENERALIZED_TIME, new Buffer('20160316182035.123Z'), 0, 19), new Date('2016-03-16T18:20:35.123Z'))
})
