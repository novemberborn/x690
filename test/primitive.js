...
test('throws when decoding with a non-number offset', t => {
  const err = t.throws(() => decode(BOOLEAN, new Buffer(0), false, 1), AssertionError)
  t.is(err.message, 'Parameter `offset` must be >= 0')
})

test('throws when decoding with a negative offset', t => {
  const err = t.throws(() => decode(BOOLEAN, new Buffer(0), -1, 1), AssertionError)
  t.is(err.message, 'Parameter `offset` must be >= 0')
})

test('throws when decoding with a non-number length', t => {
  const err = t.throws(() => decode(BOOLEAN, new Buffer(0), 0, false), AssertionError)
...
test('throws when decoding with a negative length', t => {
  const err = t.throws(() => decode(BOOLEAN, new Buffer(0), 0, -1), AssertionError)
  t.is(err.message, 'Parameter `length` must be >= 1 for non-NULL and non-OCTET_STRING primitives')
})
...
;[
  ...
  test(`throws when decoding ${title}`, t => {
    const err = t.throws(() => decode(type, new Buffer(0), 0, 1), TypeError)
    t.is(err.message, 'Unrecognized primitive, universal ASN.1 type')
  })
})

test('starts decoding from the offset', t => {
  t.true(decode(BOOLEAN, new Buffer([0b0, 0b11111111]), 1, 1))
})

test('decodes NULL', t => {
  t.is(decode(NULL, new Buffer(0), 0, 0), null)
})

test('throws when decoding a NULL with a non-zero length', t => {
  const err = t.throws(() => decode(NULL, new Buffer(0), 0, 1), AssertionError)
  t.is(err.message, 'Parameter `length` must be 0 for NULL primitives')
})

test('decodes BOOLEAN', t => {
  t.false(decode(BOOLEAN, new Buffer([0b0]), 0, 1))
  t.true(decode(BOOLEAN, new Buffer([0b11111111]), 0, 1))
})

test('throws when decoding a BOOLEAN longer than 1 byte', t => {
  const err = t.throws(() => decode(BOOLEAN, new Buffer([0b0, 0b0]), 0, 2), AssertionError)
...
test('throws when decoding an OCTET_STRING with a non-number length', t => {
  const err = t.throws(() => decode(OCTET_STRING, new Buffer(0), 0, false), AssertionError)
...
test('throws when decoding an OCTET_STRING with a negative length', t => {
  const err = t.throws(() => decode(OCTET_STRING, new Buffer(0), 0, -1), AssertionError)
  t.is(err.message, 'Parameter `length` must be >= 0 for OCTET_STRING primitives')
})
...
test('decodes OBJECT_IDENTIFIER', t => {
  t.is(decode(OBJECT_IDENTIFIER, new Buffer([0b00101010, 0b11]), 0, 2), '1.2.3')
})

test('decodes RELATIVE_OID', t => {
  t.is(decode(RELATIVE_OID, new Buffer([0b00101010, 0b11]), 0, 2), '42.3')
})
...
;[
  ...
  test(`decodes ${title}`, t => {
    t.is(decode(type, new Buffer('abc'), 0, 3), 'abc')
  })
})
